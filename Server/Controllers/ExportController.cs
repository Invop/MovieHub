...
public class ExportController : Controller
{
        ...
        where T : class
    {
        if (query != null)
        {
            if (query.ContainsKey("$expand"))
            {
                ...
                foreach (var p in propertiesToExpand) items = items.Include(p);
            }
            ...
            if (query.ContainsKey("$select")) return items.Select($"new ({query["$select"].ToString()})");
        }
    ...
    public FileStreamResult ToCsv(IQueryable query, string fileName = null)
    {
        ...
        foreach (var item in query)
        {
            ...
            foreach (var column in columns) row.Add($"{GetValue(item, column.Key)}".Trim());
            ...
            new FileStreamResult(
                new MemoryStream(
                    Encoding.Default.GetBytes(
                        $"{string.Join(",", columns.Select(c => c.Key))}{Environment.NewLine}{sb}")), "text/csv");
