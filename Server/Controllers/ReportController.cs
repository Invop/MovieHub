...
public partial class ReportController : Controller
{
    ...
    private async Task<HttpResponseMessage> ForwardRequest(HttpClient httpClient, HttpRequest currentReqest, string url)
    {
        ...
        if (currentReqest.Method == "POST")
            using (var stream = new MemoryStream())
            {
                ...
                if (body.IndexOf("AjaxScriptManager") != -1)
                {
                    ...
                    proxyRequestMessage.Content.Headers.Add("Content-Type", new[] { currentReqest.ContentType });
                }
            }
    ...
    private static async Task WriteResponse(HttpRequest currentReqest, string url, HttpResponseMessage responseMessage,
        HttpResponse response, bool isAjax)
    {
        ...
        if (isAjax && result.IndexOf("|") != -1)
        {
            ...
            while (index < result.Length)
            {
                ...
                if (content.Contains("./ReportViewer.aspx", StringComparison.InvariantCultureIgnoreCase))
                    ...
                    content = content.Replace("ReportViewer.aspx", $"{proxyUrl}/{reportServer}/Pages/ReportViewer.aspx",
                        StringComparison.InvariantCultureIgnoreCase);
        ...
        response.Headers.Append("Content-Length", new[] { Encoding.UTF8.GetByteCount(result).ToString() });
